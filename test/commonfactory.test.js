test('deploy common', async () => {
  ...
  const DAOFactoryInstance = DAOstackMigration.migration('private').package['0.1.1-rc.23'].DAOFactoryInstance;
  ...
  const votingMachine = DAOstackMigration.migration('private').package['0.1.1-rc.23'].GenesisProtocol;
  ...
  const avatar = new web3.eth.Contract(
    require('@daostack/migration-experimental/contracts/0.1.1-rc.23/Avatar.json').abi,
    avatarAddress,
    opts
  );

  const reputation = new web3.eth.Contract(
    require('@daostack/migration-experimental/contracts/0.1.1-rc.23/Reputation.json').abi,
    reputationAddress,
    opts
  );
